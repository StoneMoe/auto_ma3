...
def update_screen_data():
    ...
    screen_data = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGBA2BGR)


def locate_img(filename, area, threshold=None):
    """
    匹配指定图像并返回坐标, 匹配前先 update_screen_data()
    :param filename:
    :param area: 在指定的屏幕中绝对坐标区域进行定位，使用 X11 坐标 (left, upper, right, lower)
    :param threshold: threshold
    :return: X11 坐标, (x, y, x_center, y_center, x_end, y_end)
    """
    result_valid = False
    # OpenCV output status
    font_color = (0, 0, 255)
    offset_x = offset_y = 0
    if area != (0, 0, x11_x, x11_y):
        # offset
        offset_x = area[0] - emulator_area[0]
        offset_y = area[1] - emulator_area[1]
    # X11 area to PIL area
    ...
    mn, mx, min_loc, _ = cv2.minMaxLoc(result)
    # threshold
    if threshold is None:
        threshold = 0.2
    if mn > threshold:
        print('[CV]Mismatch: min %s | max %s | need_score %s | %s' % (mn, mx, threshold, filename))
    else:
        print('[CV]Matched: min %s | max %s | need_score %s | %s' % (mn, mx, threshold, filename))
        result_valid = True
        font_color = (0, 255, 0)

    # Draw the rectangle to OpenCV output:
    # Extract the coordinates of our best match
    mp_x, mp_y = min_loc
    # Extract target size
    trows, tcols = small_image.shape[:2]
    # Draw rect on larger image and put text
    cv2.rectangle(large_image, (mp_x, mp_y), (mp_x + tcols, mp_y + trows), font_color, 2)
    ...
    # return
    if result_valid:
        # PIL coordinates to X11 coordinates, and calc with area offset
        mp_x = mp_x / DPI_times
        mp_y = mp_y / DPI_times
        mp_x += offset_x
        mp_y += offset_y
        return LocatedImgX11Coordinates((mp_x, mp_y),
                                        (mp_x + (tcols / DPI_times / 2), mp_y + (trows / DPI_times / 2)),
                                        (mp_x + (tcols / DPI_times), mp_y + (trows / DPI_times))
                                        )
    else:
        return None
...
def is_in_instance_map():
    # 是否在副本里 Optimized
    print('[检查]是否在副本中')
    if locate_img('taskmap_exit_button.png', area_offset(930, 120, 180, 400), threshold=0.2) and not is_in_main_page():
        print('[状态]正在副本中')
        return True
    else:
        print('[状态]不在副本中')
...
def is_in_battle_result():
    print('[检查]是否在结算界面')
    if locate_img('battle_result.png', area_offset(560, 0, 0, 300), threshold=0.2):
        print('[状态]正在结算界面')
        return True
    else:
        print('[状态]不在结算界面中')
...
def is_in_store_page():
    # 是否在商店页
    print('[检查]是否在商店页中')
    button = locate_img('store_buy_button.png', area_offset(670, 340, 0, 100), threshold=0.1)
...
def is_in_detail_page():
    # 是否在详情页 Optimized
    print('[检查]是否在详情页中')
    button = locate_img('close_detail_button.png', area_offset(930, 0, 0, 500), threshold=0.1)
    button2 = locate_img('close_dialog_button.png', area_offset(930, 0, 0, 500), threshold=0.05)
...
def is_in_chat_page():
    # 是否在对话页面 Optimized
    print('[检查]是否在对话页面中')
    button = locate_img('chat_page.png', area_offset(0, 400, 950, 0), threshold=0.35)
    if button is not None:
        print('[操作]在对话页面中')
        return True
    else:
...
def is_energy_enough():
    # 检查体力
    print('[检查]体力状态')
    if not locate_img('energy_not_enough.png', emulator_area, threshold=0.2):
        print('[状态]体力充足')
        return True
    else:
        print('[状态]体力不足')
...
def has_combine_skill_button():
    # 合体技是否准备好
    print('[检查]合体技状态')
    if locate_img('combine_skill_ok.png', area_offset(640, 260, 220, 190), threshold=0.08):
        print('[状态]合体技准备就绪')
        return True
    else:
        print('[状态]合体技未准备好')
...
def has_action_button():
    # 互动按钮
    print('[检查]是否存在互动按钮')
    button = locate_img('action_button.png', area_offset(655, 260, 280, 210), threshold=0.05)
    button2 = locate_img('action_button2.png', area_offset(655, 260, 280, 210), threshold=0.05)
    button3 = locate_img('action_button3.png', area_offset(655, 260, 280, 210), threshold=0.05)
...
def has_chat_skip_button():
    # 剧情跳过按钮 Optimized
    print('[检查]是否存在剧情跳过按钮')
    button = locate_img('chat_skip_button.png', area_offset(1070, 0, 40, 610), threshold=0.15)
    if button is not None:
        print('[状态]存在跳过按钮')
        return True
    else:
...
def has_analyze_button():
    # 怪物解析器 Optimized
    print('[检查]是否存在解析器按钮')
    button = locate_img('analyze_button.png', area_offset(732, 540, 260, 45), threshold=0.005)
    if button is not None:
        print('[状态]存在跳过按钮')
        return True
    else:
...
def auto_task_target():
    # 自动寻路 Optimized
    # 仅选择日常和主线
    print('[检查]任务列表中的 日常 任务')
    tag = locate_img('daily_task_tag.png', task_area)
    tag_active = locate_img('daily_task_tag_active.jpg', task_area)
    if tag or tag_active:
        tag = tag_active if tag_active else tag
        print('[操作]点击日常任务进行寻路')
...
def use_chat_button():
    print('[操作]尝试选择对话')
    button = locate_img('correct_chat_choice.png', area_offset(730, 0, 0, 0))
    if button:
...
end_y = 0
try:
    start_x, start_y = locate_img('mumu_locate.png', (0, 0, x11_x, x11_y)).start_point
    end_x, end_y = locate_img('mumu_locate_end.png', (0, 0, x11_x, x11_y)).end_point
except AttributeError:
    print('[错误]无法定位到模拟器位置')
    exit()
emulator_area = (start_x, start_y + EMULATOR_HEADER, end_x, end_y - EMULATOR_BOTTOM)
task_area = area_offset(983, 0, 0, 450)
...
while True:
    ...
    if is_in_instance_map():  # 是否在副本中
        if has_combine_skill_button():
            use_combine_skill()
            continue
    elif is_in_chat_page():  # 在对话界面
